...
function refreshHighlights() {
  chrome.storage.local.get({ nameGroups: [], keywords: [], colors: DEFAULT_COLORS }, data => {
    ...
    data.nameGroups.forEach(group => {
      ...
      if (regexFull) {
        walkAndHighlight(regexFull, data.colors.full || DEFAULT_COLORS.full, HIGHLIGHT_CLASSES.full, group.name);
      }
      if (regexLast) {
        walkAndHighlight(regexLast, data.colors.last || DEFAULT_COLORS.last, HIGHLIGHT_CLASSES.last, group.name);
      }
    });
